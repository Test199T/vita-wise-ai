...
const ACTIONS = [
  { id: "health-analysis", icon: IconPencil, label: "วิเคราะห์สุขภาพ" },
  { id: "advice", icon: IconBook, label: "ให้คำแนะนำ" },
  { id: "lab-results", icon: IconCode, label: "แปลผลตรวจ" },
  { id: "consultation", icon: IconHeart, label: "ปรึกษาสุขภาพ" },
];
...
export default function Ai04({
  ...
  return (
    <div className="mx-auto flex w-full flex-col items-center gap-4">
      <h1 className="max-w-2xl text-pretty text-center font-heading font-semibold text-[29px] text-foreground tracking-tighter sm:text-[32px] md:text-[46px]">
        <DecryptedText
          ...
          {attachedFiles.length > 0 && (
            <div className="relative flex w-fit items-center gap-2 mb-2 overflow-hidden">
              {attachedFiles.map((file) => (
                <Badge
                  variant="outline"
                  className="group relative h-7 max-w-48 cursor-pointer overflow-hidden text-[13px] transition-colors hover:bg-accent pl-1 pr-7"
                  key={file.id}
                >
                  <span className="flex h-full items-center gap-1.5 overflow-hidden font-normal">
                    ...
                    <span className="inline overflow-hidden truncate transition-all">
                      {file.name}
                    </span>
                  </span>
                  <button
                    className="absolute right-1 top-1/2 -translate-y-1/2 z-10 rounded-sm p-1 text-muted-foreground opacity-0 focus-visible:bg-accent focus-visible:opacity-100 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-background group-hover:opacity-100 hover:bg-background"
      ...
      <div className="max-w-2xl flex-wrap gap-3 flex min-h-0 shrink-0 items-center justify-center">
        {ACTIONS.map((action) => (
          <Button
