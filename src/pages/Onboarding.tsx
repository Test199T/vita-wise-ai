...
const Onboarding = () => {
  ...
  React.useEffect(() => {
    ...
    const checkAuth = () => {
      if (!tokenUtils.isLoggedIn()) {
        console.log(
          "🚫 Onboarding: ผู้ใช้ไม่ได้เข้าสู่ระบบ - เปลี่ยนไปยังหน้า login"
        );
        navigate("/login");
        return;
      }
      ...
      const mergedData = {
        ...
        sex:
          registrationData.gender === "other"
            ? "male"
            : (registrationData.gender as "male" | "female") ||
              onboardingData.sex,
        // คำนวณวันเกิดจากอายุ (ประมาณ)
        birthDate: registrationData.age
          ? new Date(new Date().getFullYear() - registrationData.age, 0, 1)
              .toISOString()
              .split("T")[0]
    ...
    if (registrationData && !onboardingData.isCompleted && !hasUpdatedContext) {
      ...
      if (registrationData.firstName) {
        updateOnboardingData(
          "registrationFirstName" as keyof OnboardingData,
          registrationData.firstName
        );
        console.log(
          "✅ Added registrationFirstName to context:",
          registrationData.firstName
        );
      }
      if (registrationData.lastName) {
        updateOnboardingData(
          "registrationLastName" as keyof OnboardingData,
          registrationData.lastName
        );
        console.log(
          "✅ Added registrationLastName to context:",
          registrationData.lastName
        );
      }
      ...
      console.log(
        "✅ อัพเดท context และ local state ด้วยข้อมูลจาก Register เรียบร้อยแล้ว"
      );
    }
  }, [
    registrationData,
  ...
  const handleNext = async () => {
    ...
    if (currentStep === 1) {
      ...
      const firstName =
        data.firstName?.trim() || registrationData?.firstName?.trim();
      const lastName =
        data.lastName?.trim() || registrationData?.lastName?.trim();
    ...
    if (currentStep === 2) {
      ...
      toast({
        ...
        description: `เป้าหมาย: ${
          healthGoals.find((g) => g.value === data.healthGoal)?.label
    ...
    if (currentStep === 3) {
      ...
      if (!data.height || !data.weight) {
        toast({
          ...
          description:
            "กรุณากรอกข้อมูลร่างกายให้ครบถ้วน:\n• ส่วนสูง (cm)\n• น้ำหนัก (kg)",
          variant: "destructive",
        });
        return;
      }
    ...
    if (currentStep < steps.length - 1) {
      ...
      try {
        ...
        if (registrationData) {
          console.log(
            "📝 ข้อมูลจาก Register ที่จะถูกบันทึก:",
            registrationData
          );
        }
        ...
        try {
          ...
          if (!token) {
            ...
            if (registrationData && registrationData.hasJWT) {
              console.warn(
                "⚠️ registrationData.hasJWT = true แต่ไม่มี Token ใน storage"
              );
              ...
              toast({
                ...
                description:
                  "Backend ยังไม่สามารถสร้าง JWT Token ได้\nกรุณาลองสมัครสมาชิกใหม่อีกครั้ง",
                variant: "destructive",
              });
          ...
          const onboardingDataForBackend = {
            ...
            waterIntakeGlasses:
              ((data as unknown as Record<string, unknown>)
                .waterIntakeGlasses as number) || 0,
            otherLifestyleNotes:
              ((data as unknown as Record<string, unknown>)
                .otherLifestyleNotes as string) || "",
            caffeineCupsPerDay:
              ((data as unknown as Record<string, unknown>)
                .caffeineCupsPerDay as number) || 0,
            screenTimeHours:
              ((data as unknown as Record<string, unknown>)
                .screenTimeHours as string) || "2-4",
            stressLevel:
              ((data as unknown as Record<string, unknown>)
                .stressLevel as string) || "medium",
            relaxationFrequency:
              ((data as unknown as Record<string, unknown>)
                .relaxationFrequency as string) || "1-2",
            lateMealFrequency:
              ((data as unknown as Record<string, unknown>)
                .lateMealFrequency as string) || "rarely",
          };
          ...
          if (
            !onboardingDataForBackend.firstName ||
            !onboardingDataForBackend.lastName
          ) {
          ...
          try {
            ...
            const savedProfile = await apiService.saveOnboardingData(
              onboardingDataForBackend
            );
            ...
            toast({
              ...
              description:
                "ข้อมูลจะถูกบันทึกในเครื่องชั่วคราว และจะลองบันทึกลงฐานข้อมูลอีกครั้งในภายหลัง",
              variant: "destructive",
            });
        ...
        const finalFirstName =
          data.firstName || registrationData?.firstName || "";
        ...
        if (finalFirstName && finalLastName) {
          console.log("📝 Sending name data to API service:", {
  ...
  const validateRequiredData = (): boolean => {
    ...
    if (missingFields.length > 0) {
      ...
      if (registrationData) {
        ...
        const missingPersonalFields = missingFields.filter((field) =>
  ...
  const handleSkip = () => {
    ...
    if (currentStep < steps.length - 1) {
      toast({
        title: "⏭️ ข้ามขั้นตอน",
        description: `ข้ามจาก ${steps[currentStep].title} ไป ${
  ...
  const updateData = (key: keyof OnboardingData, value: unknown) => {
    ...
    if (key === "firstName" || key === "lastName") {
      ...
      if (
        ...
        console.log(
          `🔄 ${key} is empty, using registration data:`,
          registrationValue
        );
  ...
  const renderStep = () => {
    switch (currentStep) {
      case 0:
        return (
          <Card className="max-w-2xl mx-auto bg-white/90 backdrop-blur-md border border-white/20 shadow-xl">
            <CardHeader className="text-center pb-4 pt-6">
              ...
              <CardTitle className="text-2xl text-foreground">
                ยินดีต้อนรับสู่แอปสุขภาพของคุณ!
              </CardTitle>
            ...
            <CardContent className="space-y-4 pt-0 pb-6">
              <div className="text-center space-y-3">
                <p className="text-muted-foreground">
                  การตั้งค่าจะใช้เวลาเพียง 2-3 นาที
                  และจะช่วยให้เราแนะนำคุณได้ตรงเป้าหมายมากขึ้น
                </p>
                ...
                <div className="mt-2 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                  <div className="flex items-center gap-2 text-amber-800">
                    ...
                    <span className="text-sm font-medium">
                      ข้อมูลที่จำเป็นต้องกรอกก่อนข้าม:
                    </span>
                  </div>
                ...
                <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-green-700">
                    <div>
                      ...
                      {data.firstName || registrationData?.firstName}{" "}
                      {data.lastName || registrationData?.lastName}
                    </div>
                    <div>
        ...
        return (
          <Card className="max-w-2xl mx-auto bg-white/90 backdrop-blur-md border border-white/20 shadow-xl">
            ...
            <CardContent className="space-y-6">
              <div className="space-y-4">
                <Label>
                  คุณต้องการอะไร? <span className="text-red-500">*</span>
                </Label>
                <RadioGroup
                  value={data.healthGoal}
                  onValueChange={(value) => {
                    updateData("healthGoal", value);
                    const selectedGoal = healthGoals.find(
                      (g) => g.value === value
                    );
                    if (selectedGoal) {
                      toast({
              ...
              <div className="space-y-4">
                <Label>
                  อยากเห็นผลในกี่เดือน? <span className="text-red-500">*</span>
                </Label>
                <Select
                  value={data.timeline.toString()}
                  onValueChange={(value) => {
              ...
              <div className="space-y-4">
                <Label>
                  อะไรคือเหตุผลที่คุณอยากเปลี่ยนสุขภาพของคุณ? (ไม่บังคับ)
                </Label>
                <Textarea
            ...
            <CardHeader>
              ...
              <div className="mt-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                <div className="flex items-center gap-2 text-blue-800">
                  ...
                  <span className="text-sm font-medium">
                    ข้อมูลในขั้นตอนนี้ไม่บังคับ - ข้ามได้เสมอ
                  </span>
                </div>
              </div>
            </CardHeader>
                ...
                <RadioGroup
                  ...
                  onValueChange={(value) => {
                    ...
                    toast({
                      ...
                      description:
                        exerciseLabels[value as keyof typeof exerciseLabels] ||
                        value,
                      variant: "default",
                    });
                  }}
                ...
                <Select
                  ...
                  onValueChange={(value) => {
                    ...
                    toast({
                      ...
                      description:
                        activityLabels[value as keyof typeof activityLabels] ||
                        value,
                      variant: "default",
                    });
                  }}
              ...
              <div className="space-y-4">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => {
                        ...
                        toast({
                          ...
                          description: `ความถี่: ${
                            alcoholLabels[
                              value as keyof typeof alcoholLabels
                            ] || value
                          }`,
                          variant: "default",
                        });
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    value={
                      ((data as unknown as Record<string, unknown>)
                        .waterIntakeGlasses as number) || ""
                    }
                    onChange={(e) => {
                      const value = parseInt(e.target.value);
                      updateData(
                        "waterIntakeGlasses" as keyof OnboardingData,
                        value || undefined
                      );
                      if (value && value > 0) {
                        toast({
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    onChange={(e) =>
                      updateData(
                        "otherLifestyleNotes" as keyof OnboardingData,
                        e.target.value
                      )
                    }
                  />
            ...
            <CardContent className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {medicalConditions.map((condition) => (
                    ...
                    >
                      <Checkbox
                        id={condition.value}
                        checked={data.medicalConditions.includes(
                          condition.value
                        )}
              ...
              <div className="space-y-4">
                <Label htmlFor="surgeries">
                  เคยผ่าตัดหรือมีการรักษาพิเศษ? (ไม่บังคับ)
                </Label>
                <Textarea
                  id="surgeries"
              ...
              <div className="space-y-4">
                <Label htmlFor="allergies">
                  แพ้อาหาร / แพ้ยาอะไรบ้าง? (ไม่บังคับ)
                </Label>
                <Textarea
                  id="allergies"
        ...
        return (
          <Card className="max-w-2xl mx-auto bg-white/90 backdrop-blur-md border border-white/20 shadow-xl">
            <CardHeader>
              ...
              <CardDescription>ตรวจสอบข้อมูลที่คุณได้กรอกไว้</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Badge variant="secondary">
                      {healthGoals.find((g) => g.value === data.healthGoal)
                        ?.label || "ไม่ระบุ"}
                    </Badge>
                  </div>
                  <div className="space-y-2">
  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-4xl fade-in relative z-10">
        {/* Progress Bar */}
        <div className="mb-8 bg-white/10 backdrop-blur-md rounded-xl p-4 border border-white/20">
